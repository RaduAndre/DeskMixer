...
// ESP32 analog input pins for 5 sliders (update to match your board)
const int analogInputs[NUM_SLIDERS] = {33, 32, 35, 34, 39};
...
// Stores the current slider values mapped to 0-1000 (= volume * 1000).
// The host divides by 1000.0 to get a float [0.0-1.0] for the audio API.
...
void readCurrentValues() {
  // Read analog sliders.
  // ESP32 ADC is 10-bit (0-1023). We map to 0-1000 so the value represents
  // volume * 1000, matching the STM32 firmware behaviour.
  for (int i = 0; i < NUM_SLIDERS; i++) {
    int raw = analogRead(analogInputs[i]);          // 0-1023
    analogSliderValues[i] = map(raw, 0, 1023, 0, 1000); // 0-1000
  }
